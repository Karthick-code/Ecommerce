...
export default function ProductList() {
  ...
  ); // Get selected category from categories slice
  ...
  useEffect(() => {
    ...
    dispatch(
      fetchProducts({ category: selectedCategory, searchQuery, skip: 0, limit })
    );
  ...
  const loadMoreProducts = () => {
    if (hasMore && !loading) {
      const newSkip = skip + limit; // Calculate new skip value
      dispatch(incrementSkip()); // Increment the skip in the Redux store
      dispatch(
        fetchProducts({
